(function () {
  ...
  const contentByNode = {
    hardware: {
      title: '하드웨어 계층',
      copy: '연산 자원, 메모리 대역폭, 인터페이스 제약이 전체 실행 성능의 상한을 만든다.',
    },
    model: {
      title: '모델 실행 계층',
      copy: '양자화, 연산자 매핑, 런타임 스케줄링이 실제 추론 지연과 안정성을 좌우한다.',
    },
    data: {
      title: '데이터 파이프라인',
      copy: '센서/설비 데이터의 정제, 스키마 정의, 시계열 구조화가 AI 적용 가능성을 결정한다.',
    },
    operation: {
      title: '운영 피드백 루프',
      copy: '배포 이후 모니터링, 피드백, 버전 관리가 모델 성능을 현장에서 지속시키는 핵심 루프다.',
    },
  };
